...
= 0.23.0 =
* **Please read before updating — some designs that publish today will be refused after this update.** Publishing from the Bynefit app checks your layout values before it saves, and this release adds several checks that were missing. Each one refuses a value that used to save and then render as something other than what you asked for. Publishing will now tell you, rather than quietly changing it, if your design uses: an **icon size** outside 8–96 or with a fraction in it; an **alignment** other than `start` or `center` on a quote, stat or call-to-action; an **embed ratio** other than `16-9`, `4-3`, `1-1` or `21-9`; an **events limit** or an **overlay opacity** that is not a whole number; a **gallery gap** that is not one of our spacing tokens; a **gallery column count** outside the range we support — that check existed but had never run, so these published clean until now; or a **block position** — column, span, row or order — that does not fit the section it sits in. That last one is the likeliest to affect you: a block set to span 12 columns inside a 4-column layout renders full-width today because we quietly shrink the span, and will now be refused instead. Nothing already published changes; you will see this the next time you save that page.
