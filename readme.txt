...
* **Fixed — the Updates panel could show a green "Up to date" it had not earned.** With no version reading to compare against, the panel, the sidebar, the overview tile and the activity log all reported everything was current. Clicking **Refresh this readout** was the quickest way to reach that state, because it cleared the reading without taking a new one. It now takes the reading, every one of those places says plainly when there isn't one or when the check failed, and a refresh that could not reach us says so instead of showing a green tick.
* **Fixed — the activity log claimed "Up to date" on a Bynefit-hosted site with an update pending.** It was reporting whether there was anything for you to do rather than whether an update existed. It now reads "Update queued", which is what the Updates panel says.
...
* **Security:** when the plugin cannot verify an update's checksum it now records that it skipped the check — in every case where it skips, including the one that used to pass silently — and that record cannot be tampered with by the value that caused it.
