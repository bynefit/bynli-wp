...
= 0.23.0 =
* **Please read before updating — some designs that publish today will be refused after this update.** Publishing from the Bynefit app checks your layout values before it saves. This release adds four checks that were missing, and each one refuses a value that used to save and then render as something else. If your design uses an **icon size** outside 8–96, an **alignment** other than left or centre on a quote, stat or call-to-action, an **embed ratio** other than 16:9, 4:3, 1:1 or 21:9, or a **fractional events limit**, publishing will now tell you instead of quietly changing it. Nothing already published changes; you will see it the next time you save that page.
...
* **Fixed — the Updates panel could show a green "Up to date" it had not earned.** With no version reading to compare against, the panel and the sidebar both reported everything was current. Clicking **Refresh this readout** was the quickest way to reach that state, because it cleared the reading without taking a new one. It now takes the reading, and both places say plainly when there isn't one or when the check failed.
...
* **Security:** when the plugin cannot verify an update's checksum it now always records that it skipped the check, and the record cannot be tampered with by the value that caused it.
