...
* **Please read before updating — some designs that publish today will be refused after this update.** Publishing from the Bynefit app checks your layout values before it saves, and this release adds several checks that were missing. Each one refuses a value that used to save and then render as something other than what you asked for. Publishing will now tell you, rather than quietly changing it, if your design uses:
    * an **icon size** outside 8–96, or with a fraction in it;
    * an **alignment** other than `start` or `center` on a quote, stat or call-to-action;
    * an **embed ratio** other than `16-9`, `4-3`, `1-1` or `21-9`;
    * an **events limit** or an **overlay opacity** that is not a whole number;
    * a **gallery gap** that is not one of our spacing tokens;
    * a **gallery column count** outside the range we support — that check existed but had never run, so these published clean until now;
    * a **block position** — column, span, row or order — that does not fit the section it sits in, has a fraction in it, is not written as a set of breakpoints, or names a breakpoint or property we do not recognise. The last two used to be thrown away without a word, so the block landed in the default position and nothing said why; a fraction was quietly rounded down, like a position that does not fit.
* **Of those, block position is the one most likely to affect you.** A block set to span 12 columns inside a 4-column layout renders full-width today, because we quietly shrink the span to fit; from this release it is refused instead. Nothing already published changes — you will see this the next time you save that page.
* **Fixed — gallery columns were ignored and quietly changed.** The check meant to catch an out-of-range column count on a gallery had never run, so a gallery asking for more columns than we support saved cleanly and then rendered with a different number than you chose.
* **Fixed — the Updates panel could show a green "Up to date" it had not earned.** With no version reading to compare against, the panel, the sidebar and the overview tile all reported everything was current. Clicking **Refresh this readout** was the quickest way to reach that state, because it cleared the reading without taking a new one. It now takes the reading, every one of those places says plainly when there isn't one or when the check failed, and a refresh that could not reach us says so instead of showing a green tick.
* **Fixed — the sidebar, the overview tile and the activity log all claimed "Up to date" on a Bynefit-hosted site with an update pending.** They were reporting whether there was anything for you to do rather than whether an update existed, and on a Bynefit-hosted site those are different questions. All three now read "Update queued", which is what the Updates panel has always said.
...
* **Fixed — a pending update was flagged as a problem on a site that had simply not checked in recently.** The check-in time was being folded into the version verdict, so a queued update showed as an amber warning here and as an ordinary pending update everywhere else on the page. The two are now separate lines: what the version is, and when the site last checked in.
...
* **Security:** when the plugin cannot verify the checksum of one of its own updates it now records that it skipped the check — including the case that used to pass silently — and that record cannot be tampered with by the value that caused it.
